...
void transposeMatrix() {
    ...
    do {
        ...
        for (int i = 0; i < matrixRows; i++) {
            for (int j = 0; j < matrixColumns; j++) {
                printf("Enter the value for element at position (%d, %d): ", i + 1, j + 1);
                scanf("%f", &matrix1[i][j]);
            }
        }

        // Prompting the user with the matrix values they entered and checking if they are correct or not.
        printf("The matrix you have entered is: \n");
        for (int i = 0; i < matrixRows; i++) {
            for (int j = 0; j < matrixColumns; j++) {
                printf("| %8.2f ", matrix1[i][j]); // Properly spaced columns with borders
            }
            printf("|\n");
        }
    ...
    float transposedMatrix[matrixColumns][matrixRows];

    // Loop for transposing the entered matrix
    for (int i = 0; i < matrixRows; i++) {
        for (int j = 0; j < matrixColumns; j++) {
    ...
    for (int i = 0; i < matrixColumns; i++) {
        for (int j = 0; j < matrixRows; j++) {
            printf("| %8.2f ", transposedMatrix[i][j]); // Properly spaced columns with borders
        }
        printf("|\n");
    }
}
